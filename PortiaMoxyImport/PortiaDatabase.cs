...
namespace PortiaMoxyImport
{
    class PortiaDatabase
    {
          ...
          public PortiaDatabase(String aDbConnection, RichTextBox aScreenTextBox, String aTradingCurrencyStoredProc, String aLastCrossRateStoredProc, String aSellRuleStoredProc)
        {
            ...
            sellRuleStoredProc = aSellRuleStoredProc;
            if (String.IsNullOrEmpty(sellRuleStoredProc))
            {
                Globals.saveErr("Portia DB:  Sell Rule Stored Procedure unavailable.");

                throw new ArgumentNullException("Portia DB: Sell Rule Stored Procedure unavailable");
            }
        ...
        public string getSellingRule( string aPortfolio)
        {
            string rtn = "0";
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(dbConnection))
                {
                    ...
                    // 1.  create a command object identifying the stored procedure
                    SqlCommand cmd = new SqlCommand(sellRuleStoredProc, conn);
                    ...
                    using (SqlDataReader rdr = cmd.ExecuteReader())
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(rtn))
                                screen.AppendText(Globals.saveErr(String.Format("\r\n " + GetCurrentMethod() + ":Can not retrieve selling rule for portfolio : {0} .", aPortfolio)));
                            else
                                screen.AppendText(String.Format("\r\nFound selling rule {0} for : {1} ", rtn, aPortfolio));
                        } // end of while loop
                    } // end of using
                } // end of outter using
            } // end of try
